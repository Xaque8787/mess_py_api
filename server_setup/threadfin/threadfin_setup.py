...
class ThreadfinWebSocketClient:
    def __init__(self, m3u_url=None, epg_url=None, host=None, port=None, thread_user=None, thread_pass=None,
        ...
        self.holder = m3u_url
        self.m3u_remaining = remaining_m3u
    ...
    def send_m3u(self):
        for url_data in self.m3u_remaining:
            try:
                # Strip special characters and use the last 5 characters of the stripped URL
                name_value = re.sub(r'^https?://|[.:/]', '', url_data)[-5:]
                ...
                print(f"Error sending M3U data: {e}")
            finally:
                ws.close()
                time.sleep(2)


def run_websocket_operations(m3u_url=None, epg_url=None, port=None, thread_user=None,
                             thread_pass=None, token=None, host=None, remaining_m3u=None):
    client = ThreadfinWebSocketClient(m3u_url=m3u_url, epg_url=epg_url, port=port, thread_user=thread_user,
                                      thread_pass=thread_pass, token=token, host=host, remaining_m3u=remaining_m3u)
    ...
    if remaining_m3u:
        try:
            # Call the send methods
            client.send_m3u()
            print("WebSocket connection closed. M3U values sent to Threadfin")
            time.sleep(2)
        except Exception as e:
            print(f"Error during WebSocket send_m3u operation: {e}")
...
def run_reload_operations(port=None, thread_user=None, thread_pass=None, host=None):
    token = tf_api(thread_user=thread_user, thread_pass=thread_pass, host=host, port=port)
    client = ThreadfinWebSocketClient(host=host, port=port, thread_user=thread_user, thread_pass=thread_pass,
                                      token=token)
    ...
    if second_setting:
        ...
        run_reload_operations(thread_user=thread_user, thread_pass=thread_pass, host=host, port=port)
